...
<script>
import { queryPendingDealPrescription } from "@/api/work.js";
export default {
  data() {
    return {
      pendingList: [],
  ...
  methods: {
    async queryPendingDealPrescription() {
      uni.showLoading({
        title: "加载中",
      });

      try {
        let res = await queryPendingDealPrescription({
  ...
  onLoad() {
    this.queryPendingDealPrescription();
  },
};
</script>
