...
<script>
import { queryCommittedPrescription } from "@/api/work.js";
export default {
  data() {
    return {
      pendingList: [],
  ...
  methods: {
    async queryCommittedPrescription() {
      uni.showLoading({
        title: "加载中",
      });

      try {
         let res = await queryCommittedPrescription({
  ...
  onLoad() {
    this.queryCommittedPrescription();
  },
};
</script>
